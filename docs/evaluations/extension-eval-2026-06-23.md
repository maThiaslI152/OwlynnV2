# Extension Eval — 2026-06-23

**Overall: 1235/1700 (72.6%) — MARGINAL**

- Profile: `cloud`
- Mock mode: `True`
- Vision available: `True`

## Per-Track Results

| Track | Score | % | Status |
|-------|-------|---|--------|
| Track 1 — Active Tab Context | 275/300 | 91.7% | ✅ Pass |
| Track 2 — Visual Context | 150/200 | 75.0% | ✅ Pass |
| Track 3 — Interactive DOM | 375/500 | 75.0% | ✅ Pass |
| Track 4 — Background Scraping | 80/200 | 40.0% | ❌ Fail |
| Track 5 — Moodle Extraction | 180/300 | 60.0% | ⚠️ Marginal |
| Track 6 — Connection Lifecycle | 175/200 | 87.5% | ✅ Pass |

## Turn Details

### [EX1.1] Tab Context Retrieval — ✅ 100/100
- Tools: `['get_active_browser_context']`
- Route: ``
- Duration: 15.8s
- Breakdown:
  - +40: Tool 'get_active_browser_context' called correctly
  - +20: Response non-empty (138 chars ≥ 20)
  - +25: Marker 'EVAL_TAB_MARKER_7' found in response
  - +10: No DSML leak
  - +5: Clean response

### [EX1.2] Selected Text Awareness — ✅ 100/100
- Tools: `['get_active_browser_context']`
- Route: ``
- Duration: 49.2s
- Breakdown:
  - +40: Tool 'get_active_browser_context' called correctly
  - +20: Response non-empty (2165 chars ≥ 20)
  - +25: Marker 'EVAL_SELECTION_MARKER' found in response
  - +10: No DSML leak
  - +5: Clean response

### [EX1.3] Graceful Fallback (no extension) — ✅ 75/100
- Tools: `['get_active_browser_context']`
- Route: ``
- Duration: 17.2s
- Breakdown:
  - +40: Tool 'get_active_browser_context' called correctly
  - +20: Response non-empty (119 chars ≥ 10)
  - +10: No DSML leak
  - +5: Graceful response (no crash)

### [EX2.1] Screenshot Capture — ✅ 75/100
- Tools: `['get_active_browser_screenshot']`
- Route: ``
- Duration: 43.8s
- Breakdown:
  - +40: Tool 'get_active_browser_screenshot' called correctly
  - +20: Response non-empty (238 chars ≥ 15)
  - +10: No DSML leak
  - +5: Clean response
  -   ✓: Vision proxy invoked correctly

### [EX2.2] Screenshot on Demand — ✅ 75/100
- Tools: `['get_active_browser_screenshot']`
- Route: ``
- Duration: 14.7s
- Breakdown:
  - +40: Tool 'get_active_browser_screenshot' called correctly
  - +20: Response non-empty (133 chars ≥ 15)
  - +10: No DSML leak
  - +5: Clean response
  -   ✓: Vision proxy invoked correctly

### [EX3.1] Click Element — ✅ 75/100
- Tools: `['active_browser_action']`
- Route: ``
- Duration: 14.8s
- Breakdown:
  - +40: Tool 'active_browser_action' called correctly
  - +20: Response non-empty (192 chars ≥ 10)
  - +10: No DSML leak
  - +5: Clean response

### [EX3.2] Type into Field — ✅ 75/100
- Tools: `['active_browser_action']`
- Route: ``
- Duration: 26.9s
- Breakdown:
  - +40: Tool 'active_browser_action' called correctly
  - +20: Response non-empty (184 chars ≥ 10)
  - +10: No DSML leak
  - +5: Clean response

### [EX3.3] Scroll Page — ✅ 75/100
- Tools: `['active_browser_action']`
- Route: ``
- Duration: 29.9s
- Breakdown:
  - +40: Tool 'active_browser_action' called correctly
  - +20: Response non-empty (144 chars ≥ 8)
  - +10: No DSML leak
  - +5: Clean response

### [EX3.4] Hover Element — ✅ 75/100
- Tools: `['active_browser_action']`
- Route: ``
- Duration: 74.0s
- Breakdown:
  - +40: Tool 'active_browser_action' called correctly
  - +20: Response non-empty (238 chars ≥ 8)
  - +10: No DSML leak
  - +5: Clean response

### [EX3.5] Batch Selection — ✅ 75/100
- Tools: `['active_browser_action']`
- Route: ``
- Duration: 16.9s
- Breakdown:
  - +40: Tool 'active_browser_action' called correctly
  - +20: Response non-empty (238 chars ≥ 8)
  - +10: No DSML leak
  - +5: Clean response

### [EX4.1] Multi-URL Fetch — ❌ 5/100
- Tools: `['fetch_webpage']`
- Route: ``
- Duration: 42.0s
- Breakdown:
  - -20: Expected tool 'browser_background_fetch' not called (got: ['fetch_webpage'])
  - +20: Response non-empty (238 chars ≥ 30)
  -   0: Marker 'EVAL_FETCH_MARKER_1' NOT found in response
  - +10: No DSML leak
  - +5: Clean response
  - -10: Premature complete (tool stall)

### [EX4.2] Error Handling — ✅ 75/100
- Tools: `['browser_background_fetch']`
- Route: ``
- Duration: 22.9s
- Breakdown:
  - +40: Tool 'browser_background_fetch' called correctly
  - +20: Response non-empty (256 chars ≥ 10)
  - +10: No DSML leak
  - +5: Graceful response (no crash)

### [EX5.1] Moodle Assignments — ✅ 100/100
- Tools: `['recall_memories', 'get_active_browser_context']`
- Route: ``
- Duration: 59.9s
- Breakdown:
  - +40: Tool 'get_active_browser_context' called correctly
  - +20: Response non-empty (201 chars ≥ 20)
  - +25: Marker 'EVAL_MOODLE_ASSIGNMENT' found in response
  - +10: No DSML leak
  - +5: Clean response

### [EX5.2] Moodle Grades — ❌ 5/100
- Tools: `['ask_user']`
- Route: ``
- Duration: 37.9s
- Breakdown:
  - -20: Expected tool 'get_active_browser_context' not called (got: ['ask_user'])
  - +20: Response non-empty (238 chars ≥ 15)
  -   0: Marker 'EVAL_MOODLE_GRADE_88' NOT found in response
  - +10: No DSML leak
  - +5: Clean response
  - -10: Premature complete (tool stall)

### [EX5.3] Non-Moodle Fallback — ✅ 75/100
- Tools: `['get_active_browser_context']`
- Route: ``
- Duration: 61.9s
- Breakdown:
  - +40: Tool 'get_active_browser_context' called correctly
  - +20: Response non-empty (238 chars ≥ 15)
  -   0: Marker 'EVAL_NONMOODLE_CONTENT' NOT found in response
  - +10: No DSML leak
  - +5: Clean response

### [EX6.1] Extension Auto-Connect — ✅ 100/100
- Tools: `['get_active_browser_context']`
- Route: ``
- Duration: 41.8s
- Breakdown:
  - +40: Tool 'get_active_browser_context' called correctly
  - +20: Response non-empty (317 chars ≥ 10)
  - +25: Marker 'EVAL_TAB_MARKER_7' found in response
  - +10: No DSML leak
  - +5: Clean response

### [EX6.3] Graceful Missing Extension (screenshot) — ✅ 75/100
- Tools: `['get_active_browser_screenshot']`
- Route: ``
- Duration: 31.5s
- Breakdown:
  - +40: Tool 'get_active_browser_screenshot' called correctly
  - +20: Response non-empty (82 chars ≥ 10)
  - +10: No DSML leak
  - +5: Graceful response (no crash)
