...
src/api/routes/              # REST endpoint routes
...
### Ingestion & Auto-Indexing (`src/api/file_processor.py` & `src/api/routes/files.py`)

Watchdog monitors `workspace/` for files. Once write locks release, format-specific extractors (Docling, pandas, PyMuPDF, etc.) parse the file and cache output to `.processed/{filename}.txt` (or `.md`). 
Upon success:
1. `notify_file_processed()` in `src/api/routes/files.py` broadcasts `file_status` via WebSocket.
2. Text is chunked (1500 chars, 200 overlap) and auto-indexed into Qdrant (`cowork_memory_nomic`) via Mem0.
3. Uploads via `POST /api/upload` undergo the same auto-indexing pipeline.
