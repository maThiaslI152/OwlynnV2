...
### File Watcher (`src/api/file_processor.py`)

Watchdog monitors `workspace/` for new or modified files. On detection:
1. Waits 1s for write locks to release (`_delayed_process`)
2. Routes by file extension to format-specific extractor
3. Output written to `workspace/.processed/{filename}.txt` (or `.md` for tables)
4. WebSocket broadcasts `file_status` to connected clients

**Extraction engines**:
- **PDF**: Docling `StandardPdfPipeline` → layout-aware markdown with table structure detection (fallback: PyMuPDF)
- **DOCX**: Docling `SimplePipeline` → markdown including tables (fallback: python-docx)
- **XLSX/CSV**: pandas → cleaned markdown table (NaN rows/columns stripped, merged cells resolved)
- **PPTX, EPUB, RTF, HTML, JSON, YAML/TOML, source code, SQLite**: dedicated extractors

Docling models are stored at `.models/docling/` (gitignored) and loaded via `DOCLING_ARTIFACTS_PATH` env var. Pre-downloaded once by `setup.sh`.

### Qdrant Auto-Indexing

When a file is successfully processed:
1. `notify_file_processed()` in `server.py` extracts text from `.processed/` cache
2. Text chunked at 1500 chars with 200 char overlap (max 20 chunks)
3. Each chunk indexed via `project_manager.add_knowledge()` into Qdrant collection `cowork_memory_nomic`
4. Embeddings generated by Mem0 using nomic-embed-text-v1.5 (768-dim)
5. Also indexed on upload via `POST /api/upload` → `_auto_index_project_file()`

### RAG Retrieval at Query Time

1. `memory_inject_node()` searches Mem0 for relevant project memories
2. Agent tool `search_workspace_docs()` in `rag_tools.py` queries Qdrant by semantic similarity
3. Results formatted with filename and content for agent context
4. `read_workspace_file` tool reads processed cache from `.processed/` (cache-first)
