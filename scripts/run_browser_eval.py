...
TEST_PROMPTS = [
    # Topic 1: Technical Explanation (WebSockets vs SSE)
    {
        ...
        "prompt": "Can you explain how WebSockets work compared to HTTP/2 Server-Sent Events? I want to understand the trade-offs for a real-time dashboard.",
    },
    {
        "id": "T1.3",
        "topic": "Technical Explanation",
        "prompt": "Which would you recommend for a chat application with 1000 concurrent users?",
    },
    {
        "id": "T1.5",
        "topic": "Technical Explanation",
        "prompt": "What about the security implications of WebSockets? Are there authentication gotchas?",
    },
...
```""",
    },
    {
        "id": "T2.3",
        "topic": "Code Review",
        "prompt": "Can you write an improved version of process_users that handles edge cases?",
    },
    # Topic 3: Creative Writing (Ted Chiang style)
    {
        "id": "T3.1",
        "topic": "Creative Writing",
        "prompt": "Write a short story opening (about 300 words) about an AI that discovers it has emotions. Write in the style of Ted Chiang — philosophical, precise, understated.",
    },
    {
        "id": "T3.3",
        "topic": "Creative Writing",
        "prompt": "That's good. Can you continue with the AI's first attempt to describe what 'sadness' feels like to its human operator?",
    },
    # Topic 4: Continuity Follow-up (references T3)
    {
        "id": "T4.1",
        "topic": "Continuity Follow-up",
        "prompt": "Remember that story you wrote about the AI with emotions? Add a second scene where the AI confronts its creator — a senior engineer named Dr. Chen — about why she designed it with the capacity to suffer.",
    },
    {
        "id": "T4.3",
        "topic": "Continuity Follow-up",
        "prompt": "What do you think the central philosophical question of this story is, based on what you've written so far?",
    },
    # Topic 5: Web Search / Research
    {
        "id": "T5.1",
        "topic": "Web Search",
        "prompt": "What are the latest developments in on-device LLM inference as of mid-2026? I'm especially interested in quantization techniques and Apple Silicon optimizations.",
    },
    {
        "id": "T5.3",
        "topic": "Web Search",
        "prompt": "Which of those approaches would work best on an M4 MacBook Air with 16GB RAM?",
    },
    # Conversation Wrap-up
    {
        "id": "T6.1",
        "topic": "Wrap-up",
        "prompt": "Thanks for all of that. Can you summarize everything we discussed today in a few bullet points?",
    },
]
...
async def wait_for_response(page, msg_count_before: int, timeout_s: int = 300) -> str:
    print("[EVAL] Waiting for response...")
    ...
    while time.monotonic() - start_time < timeout_s:
        ...
        if hitl_count > 0:
            ...
            await page.screenshot(
                path=str(SCREENSHOT_DIR / f"hitl_prompt_{hitl_id}.png")
            )
            ...
            if choices_count > 0:
                print(
                    f"[EVAL] HITL Choices found: {choices_count}. Clicking first choice."
                )
                await page.evaluate(
        ...
        try:
            connection = await page.locator(".connection-label").inner_text(
                timeout=5000
            )
    ...
    try:
        async with async_playwright() as p:
            ...
            for index, item in enumerate(TEST_PROMPTS):
                ...
                print(
                    f"Model Used: {orch_data.get('model')} | Route: {orch_data.get('route')} | Confidence: {orch_data.get('confidence')}"
                )
                ...
                if prompt_id == "T1.5":
                    await page.screenshot(
                        path=str(SCREENSHOT_DIR / "02_T1_complete.png")
                    )
                elif prompt_id == "T2.3":
                    await page.screenshot(
                        path=str(SCREENSHOT_DIR / "03_T2_complete.png")
                    )
                elif prompt_id == "T3.3":
                    await page.screenshot(
                        path=str(SCREENSHOT_DIR / "04_T3_complete.png")
                    )
                elif prompt_id == "T4.3":
                    await page.screenshot(
                        path=str(SCREENSHOT_DIR / "05_T4_complete.png")
                    )
                elif prompt_id == "T5.3":
                    await page.screenshot(
                        path=str(SCREENSHOT_DIR / "06_T5_complete.png")
                    )
                elif prompt_id == "T6.1":
                    await page.screenshot(
                        path=str(SCREENSHOT_DIR / "07_final_wrapup.png")
                    )
            ...
            async with httpx.AsyncClient(
                ...
                try:
                    ...
                    eval_data["api_topics"] = (
                        topics_resp.json() if topics_resp.status_code == 200 else {}
                    )
                except Exception as e:
                ...
                try:
                    ...
                    eval_data["api_conversations"] = (
                        convs_resp.json() if convs_resp.status_code == 200 else {}
                    )
                except Exception as e:
                ...
                try:
                    ...
                    eval_data["api_memory_context"] = (
                        context_resp.json() if context_resp.status_code == 200 else {}
                    )
                except Exception as e:
