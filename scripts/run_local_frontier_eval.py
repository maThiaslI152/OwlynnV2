...
async def wait_for_response(page, msg_count_before: int, timeout_s: int = 1200) -> str:
    ...
    while time.monotonic() - start_time < timeout_s:
        ...
        if current_time - last_print >= 10:
            ...
            print(f"\\r[EVAL] ... running ({elapsed:.0f}s / {timeout_s}s) | est. TPS: {tps_est:.1f} | tokens: {len(current_text)//4} | tools: {tools_running or 'none'}", end="", flush=True)
...
async def get_orchestration_data(page) -> dict:
    try:
        model = await page.evaluate("() => { const el = document.querySelector('.model-badge'); return el ? el.innerText : ''; }")
        route = await page.evaluate("() => { const el = document.querySelector('.route-badge'); return el ? el.innerText : ''; }")
        confidence = await page.evaluate("() => { const el = document.querySelector('.orchestration-gauge-value'); return el ? el.innerText : ''; }")
        ...
        return {"model": model, "route": route, "confidence": confidence, "tools": tools}
    except Exception as e:
...
def score_exchange(exchange: dict, expected: dict) -> dict:
    ...
    if expected_tools:
        ...
        missing_tools = [t for t in expected_tools if not any(t in exec_t for exec_t in executed_clean)]
    ...
    try:
        async with async_playwright() as p:
            ...
            for index, item in enumerate(TEST_PROMPTS):
                ...
                msg_count_before = await page.evaluate("() => document.querySelectorAll('.message-assistant').length")
                ...
                exchange = {
                    ...
                    "assistant_response": response_text[:500] + "..." if len(response_text) > 500 else response_text,
                ...
                print(f"Model Used: {exchange['model_badge']} | Route: {exchange['route']}")
                print(f"Tools Used: {exchange['executed_tools']}")
                print(f"Duration: {exchange['duration_seconds']}s | Approx TPS: {exchange['approx_tps']} t/s")
                ...
                await page.screenshot(path=str(SCREENSHOT_DIR / f"{index+1:02d}_{prompt_id}.png"))
            ...
            print(f"[EVAL] Final Score: {eval_data['final_score']} ({eval_data['score_percentage']}%)")
