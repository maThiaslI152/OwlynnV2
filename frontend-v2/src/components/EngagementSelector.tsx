import { useState, useEffect, useCallback } from 'react'
import { useAppStore } from '../state/useAppStore'
import { fetchWithAuth } from '../lib/localRunToken'

interface Engagement {
  id: string
  name: string
  client: string
  phase: string
  status: string
  created_at: string
}

export function EngagementSelector() {
  const activeEngagementId = useAppStore((s) => s.activeEngagementId)
  const setActiveEngagementId = useAppStore((s) => s.setActiveEngagementId)
  const [engagements, setEngagements] = useState<Engagement[]>([])
  const [showCreate, setShowCreate] = useState(false)
  const [newName, setNewName] = useState('')
  const [newClient, setNewClient] = useState('')
  const [newDesc, setNewDesc] = useState('')

  const loadEngagements = useCallback(async () => {
    try {
      const resp = await fetchWithAuth('/api/pentest/engagements')
      if (resp.ok) {
        const data = await resp.json()
        setEngagements(data.engagements || [])
      }
    } catch { /* non-critical */ }
  }, [])

  useEffect(() => {
    void loadEngagements()
    const interval = setInterval(loadEngagements, 15000)
    return () => clearInterval(interval)
  }, [loadEngagements])

  // Auto-show create form when no engagements exist
  useEffect(() => {
    if (engagements.length === 0 && !activeEngagementId) {
      setShowCreate(true)
    }
  }, [engagements, activeEngagementId])

  const handleSelect = async (id: string) => {
    try {
      await fetchWithAuth(`/api/pentest/engagements/${id}/activate`, { method: 'POST' })
      setActiveEngagementId(id)
    } catch { /* non-critical */ }
  }

  const handleCreate = async () => {
    if (!newName.trim()) return
    try {
      const resp = await fetchWithAuth('/api/pentest/engagements', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: newName, client: newClient, description: newDesc }),
      })
      if (resp.ok) {
        const data = await resp.json()
        setActiveEngagementId(data.engagement_id)
        setNewName(''); setNewClient(''); setNewDesc(''); setShowCreate(false)
        void loadEngagements()
      }
    } catch { /* non-critical */ }
  }

  return (
    <div className="engagement-selector">
      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
        <select
          className="engagement-dropdown"
          value={activeEngagementId || ''}
          onChange={(e) => e.target.value && handleSelect(e.target.value)}
        >
          <option value="">Select engagement...</option>
          {engagements.map((eng) => (
            <option key={eng.id} value={eng.id}>
              {eng.name} [{eng.phase}]
            </option>
          ))}
        </select>
        <button
          type="button"
          onClick={() => setShowCreate(!showCreate)}
          style={{
            padding: '4px 8px', borderRadius: 4,
            background: showCreate ? 'rgba(233,69,96,0.15)' : 'rgba(76,175,80,0.15)',
            border: `1px solid ${showCreate ? 'rgba(233,69,96,0.25)' : 'rgba(76,175,80,0.25)'}`,
            color: showCreate ? '#e94560' : '#4caf50',
            fontSize: 11, cursor: 'pointer',
          }}
        >
          {showCreate ? '✕' : '+'}
        </button>
      </div>
      {showCreate && (
        <div className="engagement-create-form">
          <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Engagement name" />
          <input value={newClient} onChange={(e) => setNewClient(e.target.value)} placeholder="Client (optional)" />
          <textarea value={newDesc} onChange={(e) => setNewDesc(e.target.value)} placeholder="Description (optional)" rows={2} />
          <button
            type="button"
            onClick={handleCreate}
            disabled={!newName.trim()}
            style={{
              width: '100%', padding: '4px 0', borderRadius: 4,
              background: 'rgba(76,175,80,0.15)', border: '1px solid rgba(76,175,80,0.25)',
              color: '#4caf50', fontSize: 11, cursor: 'pointer',
              opacity: newName.trim() ? 1 : 0.5,
            }}
          >
            Create Engagement
          </button>
        </div>
      )}
    </div>
  )
}
