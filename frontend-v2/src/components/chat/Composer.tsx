...
export function Composer({ onSend, onStop, disabled, isGenerating, compact, hitlBlocked, onRegisterWorkspaceAttach }: ComposerProps) {
  ...
  useEffect(() => {
    ...
    const handleClear = () => setAttachedFiles([])
    
    EventBus.on('FOCUS_COMPOSER', handleFocus)
    EventBus.on('CLEAR_ATTACHMENTS', handleClear)
    
    return () => {
      EventBus.off('FOCUS_COMPOSER', handleFocus)
      EventBus.off('CLEAR_ATTACHMENTS', handleClear)
    }
  }, [])
    ...
    if (typeof window !== 'undefined') {
      ...
      if (SpeechRecognition) {
        ...
        recognitionRef.current.onresult = (event: any) => {
          ...
          setValue(prev => {
            // Very naive concatenation, a real app would track interim vs final
            const suffix = transcript.trim()
            return prev + (prev && !prev.endsWith(' ') ? ' ' : '') + suffix
          })
        }
  ...
  const onDrop = useCallback(async (acceptedFiles: File[], _fileRejections: any[], e: any) => {
    e.preventDefault()
  ...
  return (
    <div 
      {...getRootProps()}
      className={`composer-wrapper${compact ? ' composer-wrapper-compact' : ''} ${isDragActive ? 'drag-over' : ''}`}
    >
      <input {...getInputProps()} />
      ...
      <form className="composer" onSubmit={handleSubmit}>
        ...
        <div className="composer-input-row">
          ...
          <button
            type="button"
            className={`composer-mic ${isListening ? 'active text-red-500' : 'text-gray-400'}`}
