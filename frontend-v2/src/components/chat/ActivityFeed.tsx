            ...
            {group.items.map((item) => (
              <div key={item.id}>
                {item.type === 'agent_message' ? (
                  <div className="feed-agent-item" style={{ position: 'relative' }}>
                    <div style={{ fontSize: 10, opacity: 0.4, marginBottom: 2 }}>
                      Agent · {new Date(item.ts).toLocaleTimeString()}
                    </div>
                    <div style={{ paddingRight: 24 }}>{item.summary}</div>
                    <button 
                      onClick={() => handleTTS(item.id, item.summary || '')}
                      style={{ 
                        position: 'absolute', right: 4, top: 4, 
                        background: 'none', border: 'none', cursor: 'pointer',
                        color: speakingId === item.id ? '#3b82f6' : '#9ca3af' 
                      }}
                      title={speakingId === item.id ? 'Stop reading' : 'Read aloud'}
                    >
