...
code = re.sub(r"(_cap_budget_to_context\([^,]+,\s*[^)]+)\)", r"\1, max_context)", code)

# 6. Remove complex-vision and complex-longctx from complex_llm_node
code = re.sub(
    r'            elif route == "complex-vision":.*?model_label = "medium-vision"',
    "",
    code,
    flags=re.DOTALL,
)
code = re.sub(
    r'            elif route == "complex-longctx":.*?model_label = "medium-longctx"',
    "",
    code,
    flags=re.DOTALL,
)
