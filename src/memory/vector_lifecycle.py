...
class VectorLifecycleManager:
    ...
    def on_file_deleted(workspace_dir: str, filename: str):
        ...
        try:
            ...
            if len(parts) >= 2 and parts[0] == "projects":
                project_id = parts[1]
            elif workspace_dir == str(WORKSPACE_DIR) or workspace_dir == str(WORKSPACE_DIR) + "/":
                project_id = "default"
            else:
                return
        except ValueError:
    ...
    async def index_processed_file(project_id: str, filename: str, text: str):
        ...
        logger.info("Auto-indexed %d chunks of %s into project %s", len(chunks[:max_chunks]), filename, project_id)
    ...
    def delete_project_cascade(project_id: str):
        ...
        if memory is not None:
            try:
                ...
                logger.warning(f"Failed to clear Mem0 vectors for project {project_id}: {e}")
