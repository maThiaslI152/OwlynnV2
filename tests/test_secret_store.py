...
class TestSecretStore:
    ...
    def test_resolve_from_env_var(self):
        ...
        with patch("src.config.settings.DEEPSEEK_API_KEY", "env-api-key"), \
             patch("src.memory.user_profile.get_profile", return_value={"deepseek_api_key": ""}):
    ...
    def test_resolve_from_profile_fallback(self):
        ...
        self._mock_keyring(None)
        with patch("src.config.settings.DEEPSEEK_API_KEY", ""), \
             patch("src.memory.user_profile.get_profile", return_value={"deepseek_api_key": "profile-key"}):
            from src.config.secret_store import resolve_deepseek_api_key
    ...
    def test_delete_removes_from_keychain(self):
        ...
        with patch("src.config.secret_store._clear_profile_key"):
            ...
            mock_kr.delete_password.assert_called_once_with("com.owlynn.deepseek", "deepseek_api_key")
    ...
    def test_verify_valid_key(self):
        ...
        with patch.object(httpx.Client, "post", return_value=mock_response), \
             patch("src.memory.user_profile.get_profile", return_value={
