...
async def run():
    ...
    async with httpx.AsyncClient(timeout=10) as c:
        r = await c.post(
            f"{APP_BASE_URL}/api/projects", json={"name": f"SendTest {suffix}"}
        )
        project = r.json()
    ...
    try:
        async with async_playwright() as p:
            ...
            item = (
                ...
                .filter(has_text=project["name"])
                .first
            )
            await item.scroll_into_view_if_needed()
            await item.click()
            await page.get_by_text(f"Active: {project['name']}").wait_for(
            ...
            await page.get_by_text("Hello! What is 2+2?").first.wait_for(
                state="visible", timeout=15000
            )
            ...
            while time.time() < deadline:
                ...
                if msg_now > msg_before:
                    response = (
                        await page.locator(".message-assistant")
                        .nth(msg_now - 1)
                        .inner_text()
                    )
                    print(
                        f"[OK] Got response after {(time.time() - (deadline - 60)):.1f}s"
                    )
                    break
                ...
                print("[FAIL] No assistant response within 60s")
                # Show key parts
